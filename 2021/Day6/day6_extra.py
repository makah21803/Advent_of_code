...
for fish_start in list:
    ...
    for r in range(model_rows):
        row_days = [7*r + 2*i +1 for i in range(r+1)]

        if r == 0:
            row_births = [1,]
        else:
            row_births = [row_births[i-1]+row_births[i] for i in range(len(row_births))]
            row_births.append(1)
            row_births[0] = 1

        row_value = sum([b for i, b in enumerate(row_births) if row_days[i] <= fish_days])
